...
public class CheckURL {
    ...
    public static void connect_exception() throws IOException {
        if (errorIndex <= urlList.size()) {
            try {
                System.out.println("페이지 인증서 만료 : " + driver.getCurrentUrl());
                error_urlList.add(driver.getCurrentUrl());
                error_summary.add("페이지 인증서 만료");
                error_code.add(driver.findElement(By.id("error-code")).getText());
                //에러 url, 코드, 요약을 각각의 리스트에 넣어줌, 인증서 만료 페이지는 id로 코드 체크해서 넣었음
                errorIndex++;
                driver.navigate().to(urlList.get(errorIndex));
